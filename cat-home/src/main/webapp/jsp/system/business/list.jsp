	...
	<div>
	    ...
	    <h4 class="text-center text-danger">业务大盘标签会默认进行基线告警</h4>
     	<table class="table table-striped table-condensed table-bordered table-hover">
	     	...
	     	<c:forEach var="configEntry" items="${model.config.businessItemConfigs}">
	     		<c:set var="config" value="${configEntry.value}"/>
     			<tr>
				...
				<i class="ace-icon fa fa-trash-o bigger-120"></i></a>
	     			<a href="?op=alertRuleAdd&key=${config.id}&domain=${payload.domain}" id="alertRule" class="btn btn-primary btn-xs">告警</a>
 	     		</td>
	     		</tr>
	     	</c:forEach>
     	</table>
