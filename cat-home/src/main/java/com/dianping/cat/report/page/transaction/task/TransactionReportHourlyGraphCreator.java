...
public class TransactionReportHourlyGraphCreator {
	...
	class TransactionReportVisitor extends BaseVisitor {
		...
		private void buildNameGraph(Graph graph) {
			...
			for (int i = 0; i < 60; i++) {
				countValue += m_currentNameCount[i];
				sumValue += m_currentNameSum[i];
				failsValue += m_currentNameFails[i];
		...
		private void buildTypeGraph(Graph2 graph) {
			Integer[] count = parseToInteger(graph.getCount());
			Integer[] fails = parseToInteger(graph.getFails());
			Double[] sum = parseToDouble(graph.getSum());
			Double[] avg = parseToDouble(graph.getAvg());
			...
			for (int i = 0; i < 60; i++) {
				countValue += m_currentTypeCount[i];
				sumValue += m_currentTypeSum[i];
				failsValue += m_currentTypeFails[i];
