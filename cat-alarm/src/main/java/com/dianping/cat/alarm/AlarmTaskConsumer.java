...
import com.dianping.cat.alarm.entity.ServerAlarmRuleConfig;
import com.dianping.cat.alarm.transform.DefaultSaxParser;
				...
				if (rule != null) {
					...
					if (task == null) {
						try {
							...
							task = new AlarmTask(rule, category);
							Pair<List<Rule>, Long> pair = buildDuration(ruleConfig);

							task.addRules(pair.getKey());
						} catch (Exception e) {
							Cat.logError(e);
						}
					} else {
