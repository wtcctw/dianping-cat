...
public class ServerAlarm extends ContainerHolder implements Task {
	...
	private Map<Integer, RuleEndPointEntity> buildRuleEndpoints() {
		...
		for (Entry<String, List<ServerAlarmRule>> entry : rules.entrySet()) {
			...
			for (String endpoint : endPoints) {
				ServerAlarmRule rule = findRule(endpoint, entry.getValue());
	...
	public void run() {
		...
		while (active) {
			...
			try {
				Map<Integer, RuleEndPointEntity> ruleEndpoints = buildRuleEndpoints();
